...
public class ReportController {
    ...
    public record HodCoursePerformanceDTO(String courseTitle, Long passedCount, Long failedCount, BigDecimal averageGrade) {}
    ...
    public List<DepartmentPerformanceDTO> getDepartmentPerformance() {
        String sql = "SELECT d.department_name, SUM(f.is_passed) AS number_of_students_passed, CAST(AVG(f.final_grade) AS NUMBER(10,2)) AS average_grade FROM fact_enrollment f JOIN dim_department d ON f.department_key = d.department_key WHERE f.is_passed = 1 GROUP BY d.department_name ORDER BY d.department_name";
    ...
    public List<EnrollmentTrendDTO> getEnrollmentTrend() {
        String sql = "SELECT d.year_num || '-' || d.month_name AS year_month, SUM(f.enrollment_count) AS enrollment_count FROM fact_enrollment f JOIN dim_date d ON f.date_key = d.date_key GROUP BY d.year_num, d.month_num, d.month_name, d.year_num || '-' || d.month_name ORDER BY d.year_num, d.month_num";
    ...
    public List<TuitionRevenueDTO> getTuitionRevenue() {
        String sql = "SELECT d.department_name, CAST(SUM(f.tuition_fee) AS NUMBER(15,2)) as total_tuition " +
    ...
    public List<YoYEnrollmentDTO> getYoyEnrollment() {
        String sql = "WITH monthly_enrollments AS ( " +
                     "  SELECT d.year_num, d.month_num, d.month_name, SUM(f.enrollment_count) as total_enrollments " +
                     "  FROM fact_enrollment f " +
                     "  JOIN dim_date d ON f.date_key = d.date_key " +
                     "  GROUP BY d.year_num, d.month_num, d.month_name " +
                     "), " +
                     "yoy_comparison AS ( " +
                     "  SELECT year_num, month_name, month_num, total_enrollments, " +
                     "  LAG(total_enrollments, 1) OVER (PARTITION BY month_name ORDER BY year_num) as previous_year_enrollments " +
                     ...
                     "FROM yoy_comparison " +
                     "WHERE year_num = (SELECT MAX(d.year_num) FROM fact_enrollment f JOIN dim_date d ON f.date_key = d.date_key) " +
                     "ORDER BY month_num";
        return warehouseJdbcTemplate.query(sql, (rs, rowNum) -> new YoYEnrollmentDTO(rs.getString("month_name"), rs.getLong("current_year_count"), rs.getLong("previous_year_count")));
    }
    ...
    public List<HodCoursePerformanceDTO> getHodCoursePerformance(Authentication authentication) {
        String userDepartment = getDepartmentForHod(authentication);
        String sql = "SELECT c.course_title, SUM(f.is_passed) as passed_count, " +
                     " (COUNT(f.enrollment_fact_id) - SUM(f.is_passed)) as failed_count, " +
                     " CAST(AVG(f.final_grade) AS NUMBER(10,2)) as average_grade " +
                     " FROM fact_enrollment f " +
                     " JOIN dim_course c ON f.course_key = c.course_key " +
                     " JOIN dim_department d ON f.department_key = d.department_key " +
                     " WHERE d.department_name = ? AND f.final_grade IS NOT NULL " +
                     " GROUP BY c.course_title " +
                     " ORDER BY c.course_title";
        return warehouseJdbcTemplate.query(sql, (rs, rowNum) -> new HodCoursePerformanceDTO(rs.getString("course_title"),rs.getLong("passed_count"),rs.getLong("failed_count"),rs.getBigDecimal("average_grade")), userDepartment);
    }
